...
func extractingZoneId(conn *zk.ZkCli, ymb *datastructure.YMB) {
	path := "/ymb/zoneid"
...
func extractingRemoteZones(conn *zk.ZkCli, ymb *datastructure.YMB) {
...
func extractingApps(conn *zk.ZkCli, ymb *datastructure.YMB) {
	path := "/ymb/appid"
...
func extractingLoggers(conn *zk.ZkCli, ymb *datastructure.YMB) {
	path := "/ymb/loggers"
	children, err := conn.Children(path)
	if err != nil {
		log.Printf("zone [%s]: %v\n", err)
...
func extractingBrokers(conn *zk.ZkCli, ymb *datastructure.YMB) {
	path := "/ymb/brokers"
	children, err := conn.Children(path)
	if err != nil {
		log.Printf("zone [%s]: %v\n", err)
...
func extractingTopics(conn *zk.ZkCli, ymb *datastructure.YMB) {
	path := "/ymb/topics"
	children, err := conn.Children(path)
	if err != nil {
		log.Printf("zone [%s]: %v\n", err)
